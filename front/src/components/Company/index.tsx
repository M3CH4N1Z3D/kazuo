...
export default function MiEmpresa() {
  ...
  const handleAddTeamMember = async () => {
    try {
      const response = await fetch(
        `${apiUrl}/companies/${companyData?.id}/users`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/companies/${companyData?.id}/users/${editingPermissionsUser.id}/permissions`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleRemoveTeamMember = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/companies/${companyData?.id}/users/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${userToken}`,
        ...
        if (action.isConfirmed) {
          ...
          if (email) {
            const migrateResponse = await fetch(
              `${apiUrl}/companies/${companyData?.id}/users/${id}?migrateToEmail=${email}`,
              {
                method: "DELETE",
          ...
          if (confirmForce.isConfirmed) {
            const forceResponse = await fetch(
              `${apiUrl}/companies/${companyData?.id}/users/${id}?forceDelete=true`,
              {
                method: "DELETE",
                headers: { Authorization: `Bearer ${userToken}` },
              }
