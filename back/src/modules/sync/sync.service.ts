  ...
  async syncSales(salesDto: CreateSaleDto[]) {
    ...
    for (const saleDto of salesDto) {
      ...
      try {
        ...
        if (existingSale) {
          this.logger.log(
            `Sale ${saleDto.posSaleId} already exists. Skipping.`,
          );
          results.push({
            posSaleId: saleDto.posSaleId,
            status: 'skipped',
        ...
        for (const itemDto of saleDto.items) {
          ...
          let product = await queryRunner.manager.findOne(Product, {
            where: { barcode: itemDto.productId },
          });
          ...
          this.logger.log(
            `[Sync Debug] Lookup product with identifier: ${itemDto.productId}. Found: ${product ? 'Yes' : 'No'}`,
          );

          if (product) {
            this.logger.log(
              `Processing item: ${itemDto.productId}, current stock: ${product.quantity}, deducting: ${itemDto.quantity}`,
            );
            // Deduct stock
            product.quantity = Number(product.quantity) - itemDto.quantity;
            await queryRunner.manager.save(Product, product);
            this.logger.log(
              `Updated stock for product ${itemDto.productId}: ${product.quantity}`,
            );
          } else {
            this.logger.warn(
              `Product ${itemDto.productId} not found during sync. Skipping stock deduction.`,
            );
          }
        ...
        this.logger.error(
          `Failed to sync sale ${saleDto.posSaleId}: ${err.message}`,
        );
        await queryRunner.rollbackTransaction();
        results.push({
          posSaleId: saleDto.posSaleId,
