...
export class ChatBotService {
  ...
  async chat(
    ...
    try {
      ...
      console.log(
        `[ChatBot] Language requested: ${language}, using: ${selectedLanguage}`,
      );
          ...
          function_declarations: [
            {
              name: 'getStores',
              description:
                'Obtiene la lista de bodegas o tiendas disponibles del usuario actual. Devuelve ID, nombre y categoría de cada bodega.',
            },
            {
              name: 'searchProducts',
              description:
                'Busca productos en el inventario. Puede filtrar por un término de búsqueda y opcionalmente por el ID de una bodega específica.',
              parameters: {
                type: 'OBJECT',
                properties: {
                  query: {
                    type: 'STRING',
                    description:
                      '(Opcional) Término de búsqueda para el producto (nombre, descripción, etc.). Si se omite, devuelve todos los productos.',
                  },
                  storeId: {
                    type: 'STRING',
                    description:
                      '(Opcional) El ID de la bodega donde buscar. Si no se provee, busca en todas o pide aclaración.',
                  },
                },
              },
            },
            {
              ...
              description:
                'Obtiene estadísticas generales de los productos o de una bodega específica.',
              parameters: {
                type: 'OBJECT',
                properties: {
                  storeId: {
                    type: 'STRING',
                    description:
                      '(Opcional) El ID de la bodega para obtener estadísticas específicas.',
                  },
                },
              },
            },
      ...
      while (functionCalls && functionCalls.length > 0) {
        ...
        const functionResponses = await Promise.all(
          functionCalls.map(async (call) => {
            ...
            console.log(
              `Ejecutando herramienta: ${name} con argumentos:`,
              args,
            );
            ...
            if (name === 'getStores') {
              ...
              if (userId) {
                try {
                  ...
                  apiResponse = {
                    error:
                      'No se encontraron bodegas o hubo un error al buscarlas.',
                  };
                }
              } else {
                apiResponse = {
                  error: 'Usuario no identificado para buscar bodegas.',
                };
              }
                ...
                if (storeId) {
                  ...
                  if (!isValidId) {
                    ...
                    if (userId) {
                      try {
                        const userStores =
                          await this.storeService.findAllStores(userId);
                        // Buscar coincidencia exacta o si el nombre contiene el término
                        const foundStore = userStores.find(
                          (s) =>
                            s.name.toLowerCase() === storeId.toLowerCase() ||
                            s.name
                              .toLowerCase()
                              .includes(storeId.toLowerCase()),
                        );
                        ...
                        if (foundStore) {
                          ...
                          apiResponse = {
                            error: `No pude encontrar una bodega llamada "${storeId}".`,
                          };
                        }
                      } catch (e) {
                        // Si findAllStores falla (ej: 404), asumimos que no hay tiendas
                        apiResponse = {
                          error: `No encontré bodegas asociadas para buscar "${storeId}".`,
                        };
                      }
                    } else {
                      apiResponse = {
                        error: `El ID de bodega "${storeId}" no es válido.`,
                      };
                    }
                  }
                  ...
                  if (isValidId) {
                    console.log(
                      `[DEBUG] Buscando productos para storeId: ${storeId}`,
                    );
                    const products =
                      await this.productService.getProductsByStoreId(storeId);
                    console.log(
                      `[DEBUG] Encontrados ${products.length} productos en la base de datos para la bodega ${storeId}`,
                    );

                    if (
                      query &&
                      query.trim() !== '' &&
                      query.toLowerCase() !== 'todo'
                    ) {
                    ...
                    console.log(
                      `[DEBUG] Resultados enviados: ${Array.isArray(apiResponse) ? apiResponse.length : 0}`,
                    );
                  }
                } else {
              ...
              try {
                ...
                apiResponse = {
                  ...
                  message: storeId
                    ? `Estadísticas para la bodega ${storeId}`
      ...
      throw new InternalServerErrorException(
        'Error al procesar la solicitud del chatbot.',
      );
    }
  }
}
