...
export class MailService {
  ...
  async sendPasswordReset(email: string, name: string, token: string) {
    ...
    return this.sendMail(email, 'Restablecer Contraseña', 'Restablecer Contraseña', html);
  }

  async sendProductNotification(email: string, productName: string, action: string, details: string) {
    const html = productNotificationTemplate(productName, action, details);
    return this.sendMail(email, 'Actualización de Producto', `El producto ${productName} ha sido ${action}`, html);
  }
  ...
  async sendReport(email: string, message: string, attachmentBuffer: Buffer, filename: string) {
